...
const Tasks = () => {
  ...
  useEffect(() => {
    ...
    const fetch_current = async () => {
      ...
      const cur = await axios.get(
        "http://localhost:5000/api/task/current_tasks"
      );
      const comp = await axios.get(
        "http://localhost:5000/api/task/completed_tasks"
      );
  ...
  const mark = async (id) => {
    ...
    try {
      const update_task = await axios.post(
        "http://localhost:5000/api/task",
        task[0],
        config
      );
      const del = await axios.delete(`http://localhost:5000/api/task/${id}`);
            ...
            current_tasks.map((task) => {
              ...
              return (
                <div className="card p-3 m-4" key={task._id}>
                  ...
                  <h3>
                    {" "}
                    <span className="ff">Category</span> : {task.category}
                  </h3>
                  {task.duedate && (
                    <h3>Due Date : {task.duedate.slice(0, 10)}</h3>
                  )}
                  <h3>
                    {" "}
                    <span className="ff">Duration</span> : {task.duration} min
                  </h3>
                  <h3>
                    {task.description && (
                      <h3>
                        {" "}
                        <span className="ff">About Task</span> :{" "}
                        {task.description}
                      </h3>
                    )}
                  </h3>
